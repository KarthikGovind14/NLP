...
hyperparams = config.hyper_parameter()
...
# Function to create a language object with vocabulary and mapping
def get_lang(train_raw, valid_raw, test_raw):
...
# Function to collate and preprocess data batches
def collate_fn(data):
    def merge(sequences):
...
# Class to manage language vocabulary and mappings
class Lang():
    ...
    # Function to create word to ID mapping
    def w2id(self, elements, cutoff = None, unk = True, load = True):
        if load:
            ...
            vocab = {"pad": PAD_TOKEN,  "cls": CLS_TOKEN, "sep": SEP_TOKEN}
            if unk: vocab["unk"] = UNK_TOKEN
            elements = set(elements)
            input_ids = TOKENIZER.convert_tokens_to_ids(elements)
    ...
    # Function to create label to ID mapping
...
class IntentsAndSlotsDataset(data.Dataset):
    ...
    def mapping_seq(self, data, mapper, encoding = False):
        res = []
        for seq in data:
            tmp_seq = [] 
            if encoding:
